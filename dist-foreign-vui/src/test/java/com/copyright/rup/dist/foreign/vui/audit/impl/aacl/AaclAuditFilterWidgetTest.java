...
public class AaclAuditFilterWidgetTest {
    ...
    @Before
    public void setUp() {
        controller = createMock(IAaclAuditFilterController.class);
        expect(controller.getProductFamily()).andReturn("AACL").times(2);
        expect(controller.getUsagePeriods()).andReturn(List.of(USAGE_PERIOD)).once();
        widget = new AaclAuditFilterWidget(controller);
    ...
    public void testInit() {
        ...
        verifyFilters();
        verifyButtonsLayout(widget.getComponentAt(7));
        verifyFiltersLabel(widget.getComponentAt(8), "Applied Filters:");
        verify(controller);
    }
    ...
    public void testClearFilter() {
        expect(controller.getProductFamily()).andReturn(AACL_PRODUCT_FAMILY).once();
        expect(controller.getRightsholdersByAccountNumbers(Set.of(ACCOUNT_NUMBER)))
            .andReturn(List.of(buildRightsholder())).once();
        expect(controller.getUsageBatches()).andReturn(List.of()).once();
        replay(controller);
    ...
    private void verifyFilters() {
        ...
        verifyTextField(widget.getComponentAt(5), "Event ID", "ccc-event-id-filter");
        verifyTextField(widget.getComponentAt(6), "Dist. Name", "distribution-name-filter");
    }
