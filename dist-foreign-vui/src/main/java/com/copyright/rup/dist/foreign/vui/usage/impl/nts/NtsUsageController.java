...
public class NtsUsageController extends CommonUsageController implements INtsUsageController {
    ...
    public int loadNtsBatch(UsageBatch usageBatch) {
        var userName = RupContextUtils.getUserName();
        var usageIds = getUsageBatchService().insertNtsBatch(usageBatch, userName);
        ntsUsageService.sendForGettingRights(usageIds, usageBatch.getName());
        getUsageFilterController().getWidget().clearFilter();
        return CollectionUtils.size(usageIds);
    }
    ...
    public Scenario createNtsScenario(String scenarioName, Scenario.NtsFields ntsFields, String description) {
        var scenario = ntsScenarioService.createScenario(scenarioName, ntsFields, description,
