databaseChangeLog {
    ...
    changeSet(id: '2020-01-29-00', author: 'Stanislau Rudak <srudak@copyright.com>') {
        comment("B-52329 FDA: Load classified AACL usages: add Load Classified Usages permission")

        insert(schemaName: dbCommonSchema, tableName: 'cm_permission') {
            column(name: 'cm_permission_uid', value: 'baseline-fda-load-classified-usage')
            column(name: 'permission_name', value: 'FDA_LOAD_CLASSIFIED_USAGE')
            column(name: 'permission_descr', value: 'Permission to load classified usages')
            column(name: 'cm_application_area_uid', value: 'FDA')
            column(name: 'cm_permission_type_uid', value: 'ACTION')
            column(name: 'created_by_user', value: 'system')
            column(name: 'updated_by_user', value: 'system')
        ...
        rollback {
            delete(schemaName: dbCommonSchema, tableName: 'cm_role_to_permission_map') {
                where "cm_role_uid = 'baseline-fda-distribution-specialist' and cm_permission_uid = 'baseline-fda-load-classified-usage'"
            }
            delete(schemaName: dbCommonSchema, tableName: 'cm_permission') {
                where "cm_permission_uid = 'baseline-fda-load-classified-usage'"
            }
        }
    }
}
