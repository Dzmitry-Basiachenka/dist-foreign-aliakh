databaseChangeLog {
    ...
    changeSet(id: '2020-11-25-00', author: 'Darya Baraukova <dbaraukova@copyright.com>') {
        comment("B-58562 FDA: Update the item bank detail for denied SAL usage exceptions: " +
                "add permission to update rightsholder")

        insert(schemaName: dbCommonSchema, tableName: 'cm_permission') {
            column(name: 'cm_permission_uid', value: 'baseline-fda-update-rightsholder')
            column(name: 'permission_name', value: 'FDA_UPDATE_RIGHTSHOLDER')
            column(name: 'permission_descr', value: 'Permission to update rightsholder')
            column(name: 'cm_application_area_uid', value: 'FDA')
            column(name: 'cm_permission_type_uid', value: 'ACTION')
            column(name: 'created_by_user', value: 'system')
            column(name: 'updated_by_user', value: 'system')
        ...
        rollback {
            delete(schemaName: dbCommonSchema, tableName: 'cm_role_to_permission_map') {
                where "cm_role_uid = 'baseline-fda-distribution-specialist' and cm_permission_uid = 'baseline-fda-update-rightsholder'"
            }

            delete(schemaName: dbCommonSchema, tableName: 'cm_permission') {
                where "cm_permission_uid = 'baseline-fda-update-rightsholder'"
            }
        }
    }
}
