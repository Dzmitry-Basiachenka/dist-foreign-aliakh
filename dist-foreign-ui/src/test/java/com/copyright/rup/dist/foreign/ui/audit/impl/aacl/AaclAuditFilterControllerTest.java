...
public class AaclAuditFilterControllerTest {
    ...
    public void testLoadBeans() {
        ...
        expect(rightsholderService.getFromUsages(
            eq(AACL_PRODUCT_FAMILY), eq("search"), capture(pageableCapture), isNull()))
            .andReturn(rightsholders).once();
        replay(rightsholderService);
        assertSame(rightsholders, controller.loadBeans(AACL_PRODUCT_FAMILY, "search", 0, 10, null));
    ...
    public void testGetBeansCount() {
        expect(rightsholderService.getCountFromUsages(AACL_PRODUCT_FAMILY, "searchValue")).andReturn(10).once();
        replay(rightsholderService);
        assertEquals(10, controller.getBeansCount(AACL_PRODUCT_FAMILY, "searchValue"));
        verify(rightsholderService);
    }
